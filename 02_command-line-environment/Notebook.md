...
By default, both stdout and stderr are pipe to the next program. e.g. `ls folder | cat`, no matter `folder` exists or not, cat will print the result of `ls`.
...
- *Warning about `curl | bash`*  
  ...
  *Instructions like `curl -fsSL https://example.com/install.sh | bash` downloads a script and immediately executes it, which is convenient but risky.*  
  ...
  bash install.sh  # run it if no threats
  ```
