...
- Conditional statements, use `test` or `[]`:  
...
- You can store commands in a `*.sh` file.
  ...
  **To run `lecture.sh` you have you make it executable first.** Use `ls -l lecture.sh` to see your permission to that file. Use `chmod +x lecture.sh` to add execute permission to this file.
  ...
  For safety reasons, don't execute with `lecture.sh`, it will try to find and run it in `$PATH`, if there is another program with same name in your `$PATH`, you may get unexpected result. Instead, use `./lecture.sh` to specify that your program is in the current directory.  
